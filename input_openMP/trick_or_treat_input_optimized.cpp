...
/*
 Reads a file and fills the candy vector.  The file is read sequentially, then the vector is filled in parallel.
 Parameters:
...
*/
bool read_input_openmp(const std::string &filename, int &homes, int &maxCandy, std::vector<int> &candy) {
    ...
    // Sequential reading of the file into a temporary buffer on the stack
    //int temp[10000]; // there are at most 10000 houses, so this is safe
    //Other option : Sequential reading of the file into a temporary buffer on the heap
    std::vector<int> temp(homes);
